...
/*
double newY(double oldY, double oldVelocity, double gravity) {
    double theNewY = oldY + (oldVelocity*t) + ((1.0/2)*gravity*t*t);
    return theNewY;
}

void newVelocity(double *oldVelocityAddress, double gravity) {
    double oldVelocity = *oldVelocityAddress;
    double theNewVelocity = oldVelocity + (gravity * t);
    *oldVelocityAddress = theNewVelocity; 
}

struct landerInfo updateParameters(struct landerInfo *landerAddress, double *oldVelocityAddress, double gravity) {
    struct landerInfo lander = *landerAddress;
    double oldVelocity = *oldVelocityAddress;
    lander.bottomLeftY = newY(lander.bottomLeftY, oldVelocity, gravity);
    lander.topLeftY = newY(lander.topLeftY, oldVelocity, gravity);
    lander.topRightY = newY(lander.topRightY, oldVelocity, gravity);
    lander.bottomRightY = newY(lander.bottomRightY, oldVelocity, gravity);
    
    newVelocity(oldVelocityAddress, gravity);
    return lander;
}
*/
...
void executeWithoutBlock()
{
    ...
    //eraseLander(executable, landerAddress, drawThruster);
