...
extern double aX;
extern double aY;
extern double attitude_radians;
...
double findCentre(double point1, double point2) {
    double centre = (point2 + point1) / 2.0;
    return centre;
}
...
void initializeLander() {
    ...
    lander.thrusterY = 48;
    lander.centreX = findCentre(findCentre(lander.bottomLeftX, lander.bottomRightX), findCentre(lander.topLeftX, lander.topRightX));
    lander.centreY = findCentre(findCentre(lander.bottomLeftY, lander.bottomRightY), findCentre(lander.topLeftY, lander.topRightY));
    attitude_radians = PI/2.0;
}
...
void drawLander() {
    ...
    // fprintf(executable, "drawSegment %ld %ld %ld %ld\n", lround(lander.centreX), 
    //     lround(lander.centreY), lround(lander.centreX), lround(lander.centreY));
    //flush the executable
    fflush(executable);
}
...
double rotateX(double x, double y) {
    double angle_degrees = rotationDirection * ANGLE;
...
void rotateLander() {
    ...
    tempX = rotateX(lander.bottomLeftX, lander.bottomLeftY);
    tempY = rotateY(lander.bottomLeftX, lander.bottomLeftY);
    lander.bottomLeftX = tempX;
    lander.bottomLeftY = tempY;
    tempX = rotateX(lander.topLeftX, lander.topLeftY);
    tempY = rotateY(lander.topLeftX, lander.topLeftY);
    lander.topLeftX = tempX;
    lander.topLeftY = tempY;
    tempX = rotateX(lander.topRightX, lander.topRightY);
    tempY = rotateY(lander.topRightX, lander.topRightY);
    lander.topRightX = tempX;
    lander.topRightY = tempY;
    tempX = rotateX(lander.bottomRightX, lander.bottomRightY);
    tempY = rotateY(lander.bottomRightX, lander.bottomRightY);
    lander.bottomRightX = tempX;
    lander.bottomRightY = tempY;
    tempX = rotateX(lander.thrusterX, lander.thrusterY);
    tempY = rotateY(lander.thrusterX, lander.thrusterY);
    lander.thrusterX = tempX;
    lander.thrusterY = tempY;
    ...
    // lander.centreX = (lander.bottomLeftX + lander.bottomRightX) / 2.0;
    lander.centreX = tempCentreX;
    lander.centreY = tempCentreY;
    // lander.centreY = (lander.topLeftY + lander.bottomLeftY) / 2.0;
}
...
// double newY(double oldY, double oldVelocityY) {
//     double theNewY = oldY + (oldVelocityY*t) + ((1.0/2)*gravity*t*t);
//     return theNewY;
// }
...
void updateParameters() {
   //update the x- and y-velocities
    ...
    fprintf(pipePointer, "velocity: %lg\n", vX);
    fprintf(pipePointer, "acceleration: %lg\n", aX);
    fprintf(pipePointer, "\n");
    // //update the net x- and y-accelerations
    aX = 0;
    aY = gravity;

    //update the x position components
    double displacementX = vX * t;
    lander.bottomRightX += displacementX;
    lander.topLeftX += displacementX;
    lander.topRightX += displacementX;
    lander.bottomRightX += displacementX;
    lander.thrusterX += displacementX;

    //update the y position components
    double displacementY = vY * t;
    lander.bottomLeftY += displacementY;
    lander.topLeftY += displacementY;
    lander.topRightY += displacementY;
    lander.bottomRightY += displacementY;
    lander.thrusterY += displacementY;
      ...
      if (c != ERR) {
          ...
          if (landed) {
            ...
            gameOver = 1;
              ...
              double angle_degrees = rotationDirection * ANGLE;
              double angle_radians = angle_degrees * PI / 180.0;
              attitude_radians -= angle_radians;
              thrusterOn = 0;
          } else if (c == KEY_RIGHT) {
                ...
                //update the net x- and y-accelerations
                aX = thrust * cos(attitude_radians);
                aY = gravity + (thrust * sin(attitude_radians));
                thrusterOn = 1;
          } else if (c == 'q') {
                gameOver = 1;
                break;
...
void executeWithoutBlock()
{
    ...
    // if ( ((400-5) <= lander.bottomLeftY && lander.bottomLeftY <= 400) && 
    //   ((400-5) <= lander.bottomRightY && lander.bottomRightY <= 400)) {
    //   landed = 1;
    // }
