...
Input: 
    line - a character array (string) that is a comment
Output:
    None. However, as a side effect, this function takes the comment in the
     line array and prints it to stdout
*/
...
Input:
    line - a character array (string) with the image name whose information we
     have to print
    images - an array containing elements of type struct imageInfo that holds
     all relevant information about the image to be printed
    arraySize - the current size of the dynamic array images
Output:
    desiredIndex - the index of the image in the images array which will be
     used by the main function to assess whether an an invalid input was
     encountered
    Nevertheless, as a side effect, this function prints all the points'
     (x, y) coordinate information of the image in question to stdout
     (assuming the image is defined in the images array)
*/
