...
Output:
    ...
    imageInfo array
*/
int addImage(FILE *input, char line[], struct imageInfo **imagesAddress, int arraySize, int *lineNumberAddress, int *memoryAllocatedAddress) {
    arraySize++;
    if (arraySize == 1) {
        *imagesAddress = (struct imageInfo *) malloc(arraySize*sizeof(struct imageInfo));
        *memoryAllocatedAddress = 1;
        printf("here's what memoryAllocatedAddress looks like now: %d\n", *memoryAllocatedAddress);
    } else {
    ...
    while (imageProcessed == 0) {
        ...
        sscanf(line, "%s", word);
        (*lineNumberAddress)++;
        //printf("line bro:::::%s whitespace: %d\n", line, isspace(*line));
...
void errorPrint(int lineNumber, int inputOpen, FILE *input, int executableOpen, FILE *executable, int memoryAllocated, struct imageInfo *images) {
