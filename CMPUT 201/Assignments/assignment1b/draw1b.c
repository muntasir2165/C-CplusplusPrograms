    ...
    while(!feof(input)) {
            ...
            switch (lineType){
                case 1:
                    ...
                    break;
                case 2:
                    addImage(input, line+strlen("Image "), images,
                        currentIndex);
                    currentIndex++;
                    break;
                case 3:
    ...
    while (imageProcessed == 0) {
        ...
        if (strncmp(word, "End", strlen("End")) == 0) {
            ...
            if (images[currentIndex].totalPoints == 1) {
              ...
              images[currentIndex].currentX += newX;
              images[currentIndex].currentY += newY; 
              images[currentIndex].x[images[currentIndex].totalPoints] =
              images[currentIndex].currentX;
              images[currentIndex].y[images[currentIndex].totalPoints] =
              images[currentIndex].currentY;
...
void draw(char line[], struct imageInfo images[], FILE* executable) {
    ...
    for (int i = 0; i < 5*LINE_NUM; i++) {
        if (strncmp(word, images[i].name, LINE_LEN) == 0)
            desiredIndex = i;
    }
...
void translate(char line[], struct imageInfo images[]) {
    ...
    double deltaX;
    double deltaY;
    sscanf(line, "%s%lg%lg", word, &deltaX, &deltaY);
